    ...
    for event in events {
        match event.payload {
            ...
            EventPayload::RunStatusChanged { run_id, status, .. } => {
                let run_state = run_states.entry(run_id).or_insert_with(|| RunProjectionState {
                    run_id,
            ...
            EventPayload::ProgressChunkEmitted {
                ...
                let state = run_states.entry(run_id).or_insert_with(|| RunProjectionState {
                    run_id,
                    workspace_id: Uuid::nil(),
                    status: "running".to_string(),
                    total_cost: 0.0,
            ...
            EventPayload::WorkItemAssigned {
                ...
                let item = item_states.entry(item_id).or_insert_with(|| ItemProjectionState {
                    item_id,
                    run_id,
                ...
                run_states.entry(run_id).or_insert_with(|| RunProjectionState {
                    run_id,
                    workspace_id: Uuid::nil(),
                    status: "running".to_string(),
                    total_cost: 0.0,
            ...
            EventPayload::WorkItemFailed { run_id, item_id, .. } => {
                let item = item_states.entry(item_id).or_insert_with(|| ItemProjectionState {
                    item_id,
                    run_id,
